...
class SNE:
    ...
    @staticmethod
    def _conditional_probability_p_j_i(x: torch.tensor, i: int, j: int, sigma: float = 2):
        ...
        for k in range(x.shape[1]-1):
            if i != k: 
                denominator_vec[k] = torch.exp(-torch.norm(x[:, i] - x[:, k]) / 2 / (sigma ** 2))

        return numerator / denominator_vec.sum()

    @staticmethod
    def _conditional_probability_q_j_i(y: torch.tensor, i: int, j: int, sigma: float = 2):
...
if __name__ == "__main__":
    ...
    print(f"{tensor=}")
    sne = SNE()
    y_hat = sne._kullback_leibler_2(tensor)
    print(tensor.grad)
    
    print(f"{y_hat=}")
    fig = px.scatter(x=y_hat[:, 0], y=y_hat[:, 1], color=["b"]*int(n_obs/2) + ["r"]*int(n_obs/2))
    fig.show()
